...
There is now no maximum limit to pins that may be created

ArcEye 2013 <arceyeATmgwareDOTcoDOTuk>

syntax 
loadusr -W paramsaver F(f)=NN S(s)=NN B(b)=NN filename=xxxxx onstart=0/1 onexit=0/1 

NN is number of IN and OUT pins to create for each data type
...
int sz = sizeof(struct __comp_state) + (sizeof(struct xstruct) * pins)  + __comp_get_data_size();
    ...
    if(float_pins)
        ...
        for(j=0; j < (float_pins); j++) 
           {
           r = hal_pin_float_newf(HAL_IN, &(inst->structx[j]->invalueF), comp_id,"%s.invalueF-%02d", prefix, j);
           if(r != 0) 
              return r;
           r = hal_pin_float_newf(HAL_OUT, &(inst->structx[j]->outvalueF), comp_id,"%s.outvalueF-%02d", prefix, j);
           if(r != 0) 
              return r;
           }
        }   
    ...
    if(s32_pins)
        ...
        for(j=0; j < (s32_pins); j++) 
            {
            r = hal_pin_s32_newf(HAL_IN, &(inst->structx[j]->invalueS), comp_id,"%s.invalueS-%02d", prefix, j);
            if(r != 0) 
                return r;
            r = hal_pin_s32_newf(HAL_OUT, &(inst->structx[j]->outvalueS), comp_id,"%s.outvalueS-%02d", prefix, j);
            if(r != 0) 
                return r;
            }
        }
    ...
    if(bit_pins)
        ...
        for(j=0; j < (bit_pins); j++) 
            {
            r = hal_pin_bit_newf(HAL_IN, &(inst->structx[j]->invalueB), comp_id,"%s.invalueB-%02d", prefix, j);
            if(r != 0) 
                return r;
            r = hal_pin_bit_newf(HAL_OUT, &(inst->structx[j]->outvalueB), comp_id,"%s.outvalueB-%02d", prefix, j);
            if(r != 0) 
                return r;
            }
        }           
...
FILE *fp2;
    ...
    if(fp2 != NULL)      
        ...
        printf("Error: %s does not exist", filepath);
}
...
int i;
    ...
    if(argv[1] == NULL || argc < 2) 
        {
        printf("paramsaver - Error: No pins specified\n");
        exit(0);
        }
