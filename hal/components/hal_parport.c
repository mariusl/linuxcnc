...
typedef struct {
    ...
    unsigned char use_control_in; /* non-zero if pins 1, 14, 16, 17 are input */ 
	unsigned char use_control_bi; /* pins 1, 14 ,16 , 17 are bi-directional */
...
static int export_bi_pin(int portnum, int pin, hal_bit_t ** base, int n);
...
/////////////////////////////////////////////////////////////////
    ...
    if((port->use_control_in) || (port->use_control_bi)) {
        mask = 0x01;
...
static int pins_and_params(char *argv[])
{
	...
	n++;
	if (argv[n] != 0) {
		rtapi_print ( "requested port mode = '%c'\n", argv[n][0] );
		...
		n++;
                ...
                data_dir[num_ports] = 0;
// ************************************************************
// We need to put the port in bi-directional state here not just IN
            //    use_control_in[num_ports] = 1;
				use_control_bi[num_ports] = 1;
...
//////////////////////
	if(use_control_bi[n]){
		port_data_array[n].use_control_bi = use_control_bi[n];
...
////////////////////////////////////////////
    ...
    if((port->use_control_in == 0) && (port->use_control_bi == 0))  {
rtapi_print ( "setting control port to output\n");
	/* declare output variables (control port) */
		...
		if((port->use_control_in == 1) ){
rtapi_print ( "setting control port to input\n");
...
rtapi_print ( "setting control port to bi-directional\n");
			retval += export_bi_pin(portnum, 1, port->control_in, 0);
        	retval += export_bi_pin(portnum, 14, port->control_in, 1);
        	retval += export_bi_pin(portnum, 16, port->control_in, 2);
        	retval += export_bi_pin(portnum, 17, port->control_in, 3);
		}
    }
...
static int export_bi_pin(int portnum, int pin, hal_bit_t ** base, int n)
{
    ...
    retval = hal_pin_bit_newf(HAL_IN, base + (2 * n), comp_id,
            "parport.%d.pin-%02d-bi_in", portnum, pin);
    if (retval != 0) {
	return retval;
    }
	retval = hal_pin_bit_newf(HAL_OUT, base + (2 * n) + 1, comp_id,
            "parport.%d.pin-%02d-bi_out", portnum, pin);
    if (retval != 0) {
	return retval;
    }
