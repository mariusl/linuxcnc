...
# -------------------------------------------
#####################################################################################
# amended to include an param_pin for direct HAL initialisation and setting of value
# ArcEye 22042013
#####################################################################################
...
class pyvcp_dial(Canvas):
    ...
    ##ArcEye added hal_param and param_pin to __init__ ####        
...
##ArcEye ##########################################################            
        if halparam == None:
            ...
            if self.param_pin == 1:
                halparam = "dial." + str(pyvcp_dial.n) + ".param-pin"
...
## ArcEye added - no description of fields   ####    
    ...
    <bar>
        ...
        <initval>25.6</initval>  added ArcEye 2013 set initial value 
    </bar>
    """
##################################################    
    n=0
    # FIXME logarithmic scale?
##ArcEye added initval to __init__ ####        
    def __init__(self,master,pycomp,fillcolor="green",bgcolor="grey",
               halpin=None,initval=0,min_=0.0,max_=100.0,**kw):
...
## ArcEye - amended to test initval  & set halpin to that value #####
## otherwise initval will be cancelled by 1st update     ############      
        if initval > min_ and initval < max_ :
            self.value = initval
        else:
...
## why > 0.5 = 1, anticipating analogue input?
...
##ArcEye ##########################################################       
        if halparam == None:
            self.param_pin = param_pin
            if self.param_pin == 1:
                halparam = "scale."+str(pyvcp_scale.n)+".param-pin"
                self.halparam=halparam        
                pycomp.newpin(halparam, HAL_FLOAT, HAL_IN)
###################################################################               
        pyvcp_scale.n += 1       
